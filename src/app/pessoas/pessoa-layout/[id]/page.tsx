...
export default function LayoutPessoa() {
  let pessoaId = usePathname().split("/").pop();
  ...
  async function getPessoas() {
    try {
      let response = await fetch(`http://127.0.1/api/v1/pessoaid/${pessoaId}`, {
        headers: {
      ...
      <main className="mt-10">
          ...
          <div className="flex max-w-full flex-col gap-y-6">
            ...
            <div className="flex h-[128px] flex-wrap justify-between gap-y-5 gap-x-3">
              <div className="flex flex-col gap-y-2">
                ...
                <p className="text-sm font-normal">{pessoas?.nome}</p>
              </div>
              <div className="flex flex-col gap-y-2">
                <p className="font-semibold">email</p>
                <p className="text-sm font-normal">{pessoas?.email}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mb-4 flex w-full max-w-[650px] flex-col gap-y-8 rounded-2xl bg-secondary p-6">
          ...
          <div className="flex w-full max-w-[478px] flex-col gap-y-6 rounded-2xl bg-secondary px-4 py-6">
            <h1 className="text font-semibold">Confira os valores do evento</h1>
            <p>
              Inteira: {"R$ 100,00"} <br />
